...
if (!$data_tugas) {
    pesan_flash('sukses', 'Tugas tidak ditemukan.');
    alihkan('../dashboard.php');
}

$judul = $data_tugas['judul'];
$deskripsi = $data_tugas['deskripsi'];
$mata_kuliah = $data_tugas['mata_kuliah'];
$prioritas = $data_tugas['prioritas'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($judul === '') {
        ...
        $query = "UPDATE tugas SET 
                    ...
                    mata_kuliah = '$mata_kuliah_bersih',
                    prioritas = '$prioritas_bersih',
                    deadline = " . ($deadline_bersih !== '' ? "'$deadline_bersih'" : "NULL") . "
                  WHERE id = " . (int) $data_tugas['id'] . " AND id_pengguna = " . (int) $pengguna_login['id'];

        if (mysqli_query($koneksi, $query)) {
            pesan_flash('sukses', 'Tugas berhasil diperbarui.');
            alihkan('../dashboard.php');
        } else {
            $pesan_error = 'Tugas gagal diperbarui.';
        }
    }
}
